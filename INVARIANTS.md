# Axis & Allies–Style Game Invariants

This document is the authoritative contract for **`res://core/`** (rules, state, commands, events, validators, RNG).

Assume all of this is true when working on engine code, tests, and session adapters.

For Godot presentation (`res://scenes/`, `res://scripts/`, `res://debug/`), see **Presentation layer** at the end. UI may stage or display differently, but **must not mutate authoritative state except through commands**.
...
Includes the original set plus extensions used by the depot:
...
1. **Change `core/`** → update this file and add/adjust tests under `res://tests/`.
...
- `DEBUG_RULES.md` — debugging policy
- `res://tests/integration/` — session and scene smoke tests (39 tests)
