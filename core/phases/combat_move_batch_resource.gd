extends Resource
class_name CombatMoveBatchResource

@export var command_id: String = ""
@export var faction_id: String = ""
@export var moves: Array[CombatMoveEntryResource] = []


static func from_command(cmd: Command) -> CombatMoveBatchResource:
	var batch := CombatMoveBatchResource.new()
	...
	for move_data in raw_moves:
		if typeof(move_data) != TYPE_DICTIONARY:
			continue
		batch.moves.append(CombatMoveEntryResource.from_dict(move_data))
