...
static func create(map_data: Dictionary, units_data: Dictionary, factions_data: Dictionary, setup_data: Dictionary, rules_data: Dictionary, rng_seed: Dictionary) -> GameSession:
	var s := GameSession.new()
	s._init_session(map_data, units_data, factions_data, setup_data, rules_data, rng_seed)
	return s

func _init_session(map_data: Dictionary, units_data: Dictionary, factions_data: Dictionary, setup_data: Dictionary, rules_data: Dictionary, rng_seed: Dictionary) -> void:
	state = GameState.new()
...
func apply_command(cmd_dict: Dictionary) -> Dictionary:
	...
	if not validator.validate(cmd): return {"result_type": "error", "error": validator.get_error(), "events": [], "new_state": state.to_snapshot()}
	var events: Array = []
	match cmd.type:
		Command.Type.PURCHASE_UNITS: events = _sync_seq(economy.process_purchase(cmd))
		Command.Type.MOVE_UNITS: events = _sync_seq(movement.process_move(cmd))
		Command.Type.LOAD_TRANSPORT: events = _sync_seq(movement.process_load(cmd))
		Command.Type.UNLOAD_TRANSPORT: events = _sync_seq(movement.process_unload(cmd))
		Command.Type.DESIGNATE_AMPHIBIOUS: events = _sync_seq(amphibious.designate_assault(cmd))
		Command.Type.PLACE_UNITS: events = _sync_seq(placement.process_placement(cmd))
		Command.Type.END_PHASE: events = _handle_end_phase()
		Command.Type.END_TURN: events = _handle_end_turn()
	_append(events)
	return {"result_type": "ok", "events": events.map(func(e): return e.to_dict()), "new_state": state.to_snapshot()}
...
func _handle_end_turn() -> Array:
	...
	if not state.game_over: events.append_array(_sync_seq(turn_engine.end_turn()))
	return events
...
func get_events_since(seq: int) -> Array:
	var result: Array = []
	for e in _events:
		if e.sequence > seq: result.append(e.to_dict())
	return result

func get_legal_commands(player: String) -> Array:
	if player != state.current_faction_id: return []
	match state.current_phase:
		"purchase": return ["purchase_units", "end_phase"]
		"combat_move": return ["move_units", "load_transport", "designate_amphibious", "end_phase"]
		"combat": return []
		"noncombat_move": return ["move_units", "load_transport", "unload_transport", "end_phase"]
		"mobilize": return ["place_units", "end_phase"]
		"collect_income": return ["end_turn"]
	return []

func _sync_seq(events: Array) -> Array:
	for e in events:
		if e.sequence > _seq: _seq = e.sequence
...
func _load_map(data: Dictionary) -> void:
	...
	for edge in data.adjacency:
		...
		if f not in state.adjacency: state.adjacency[f] = []
		if t not in state.adjacency: state.adjacency[t] = []
		if t not in state.adjacency[f]: state.adjacency[f].append(t)
		if f not in state.adjacency[t]: state.adjacency[t].append(f)
	...
	for entry in data.starting_units:
		...
		if ut and ut.is_container():
			for i in range(cnt):
				...
				state.transport_instances[iid] = {"instance_id": iid, "unit_type_id": utid, "cargo": [], "region_id": rid}
				state.region_units[rid].append({"faction_id": fid, "unit_type_id": utid, "instance_id": iid, "count": 1})
		else:
			var found := false
			for u in state.region_units[rid]:
				if u.faction_id == fid and u.unit_type_id == utid and not u.has("instance_id"): u.count += cnt; found = true; break
			if not found: state.region_units[rid].append({"faction_id": fid, "unit_type_id": utid, "count": cnt})