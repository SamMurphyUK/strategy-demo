...
func process_placement(command: Command) -> Array:
	...
	for p in placements:
		for u in p.units: _add_to_region(p.region_id, faction, u)
	state.pending_purchases[faction] = []
	return [GameEvent.create(GameEvent.Type.UNITS_PLACED, {"faction_id": faction, "placements": placements}, _next_seq())]
...
func check_forfeited(faction: String) -> Array:
	...
	for rid in state.regions:
		...
		if r.has_factory and r.owner_faction_id == faction: current.append(rid)
	var lost: Array = []
	for f in valid:
		if f not in current: lost.append(f)
	if lost.size() > 0 and pending.size() > 0:
		var ipc := 0
		for p in pending:
			var ut: Unit = state.unit_types.get(p.unit_type_id)
			if ut: ipc += ut.cost * p.count
		state.pending_purchases[faction] = []
		return [GameEvent.create(GameEvent.Type.PLACEMENT_FORFEITED, {"faction_id": faction, "region_id": lost[0], "reason": "factory_lost", "forfeited_units": pending, "ipc_lost": ipc}, _next_seq())]
	return []
...
func _add_to_region(rid: String, faction: String, entry: Dictionary) -> void:
	...
	if ut and ut.is_container():
		for i in range(entry.count):
			...
			state.transport_instances[iid] = {"instance_id": iid, "unit_type_id": entry.unit_type_id, "cargo": [], "region_id": rid}
			units.append({"faction_id": faction, "unit_type_id": entry.unit_type_id, "instance_id": iid, "count": 1})
	else:
		for u in units:
			if u.faction_id == faction and u.unit_type_id == entry.unit_type_id and not u.has("instance_id"): u.count += entry.count; return
